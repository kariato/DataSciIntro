def mapper(x):
    ...
    def format_key(fog, rain):
        return '{}fog-{}rain'.format(
            'no' if eval(fog)==0.0 else '',
            'no' if eval(rain)==0.0 else ''
        )
    ...
    for line in x:
        if header:
            ...
            fogColumn=headerValues["fog"]
            rainColumn=headerValues["rain"]
        else:
            subway = line.split(',')
            dataOutput.append("{0}\t{1}".format(format_key(subway[fogColumn],subway[rainColumn]),subway[entriesColumn]))
